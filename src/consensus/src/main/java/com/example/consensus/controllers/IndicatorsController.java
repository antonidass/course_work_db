...
public class IndicatorsController {
    ...
    public ResponseEntity<?> getIndicatorsById(@PathVariable(name = "id") Long id) {
        Indicators indicators;
        try {
            indicators = indicatorsService.getIndicatorsById(id);
        }  catch (FileStorageException exc) {
            return new ResponseEntity<>("Финансовые показатели у компании с id = " + id + " не найдены!", HttpStatus.BAD_REQUEST);
        }
        return new ResponseEntity<>(indicators, HttpStatus.OK);
    }
